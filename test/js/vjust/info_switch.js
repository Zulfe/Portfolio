class InfoSwitcher {
    ...
    constructor(container, UID, entries, headers) {
        ...
        EventBus.addEventListener("is_sibling_update", this.handleSiblingUpdate, this);
        
        EventBus.dispatch("module_created", this, this._UID, this);
    }

    handleSiblingUpdate(event, sibling_UID, focused_degree) {
        if(sibling_UID == this._UID)
            return;

        if(sibling_UID.split("-")[1] == this._UID.split("-")[1])
            this.switchDegreeNoAnnounce(focused_degree);
    }
    ...
    initialize(container) {
        var switcher_style =
        "<style>" +
"       #" + this._UID + "{ height: 100%; width: 100%; -webkit-backface-visibility: hidden; -webkit-transform: translateZ(0) scale(1.0, 1.0); transform: translateZ(0) }" +
"       #" + this._UID + " .rotate-container { visbility: hidden; height: 100%; width: 20%; display: inline-block; vertical-align: top;}" +
...
"       #" + this._UID + " .text-view p { position: absolute; top: 0%; left: 0%; font-size: 20vh; white-space: nowrap; margin: 0px 0px 0px 0px;}" +
    ...
    switchDegree(degree) {
        ...
        EventBus.dispatch("is_sibling_update", this._UID, this._focus);
        this.displayDegree();
    }

    switchDegreeNoAnnounce(degree) {
        if(degree === undefined)
            this._focus = (this._focus + 1) % this._num_degrees;
        else {
            if(degree < this._num_degrees)
    ...
    incrementDegree() {
        this._focus = (this._focus + 1) % this._num_degrees;
        console.log("InfoSwitcher " + this._UID + " announcing with focus " + this._focus);
        EventBus.dispatch("is_sibling_update", this, this._UID, this._focus);
        this.displayDegree();
    }
